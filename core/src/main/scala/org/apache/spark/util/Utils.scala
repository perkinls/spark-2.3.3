...
private[spark] object Utils extends Logging {
  ...
  /**
   * 在给定的父目录中创建一个临时目录。当VM关闭时，目录将被自动删除。
   */
  def createTempDir(
  ...
  /**
   * 将所有数据从InputStream复制到OutputStream。除非显式地将transferToEnabled设置为true，否则文件流到文件流复制的NIO方式在默认情况下是禁用的，
   * 应通过spark.file.transferTo = [true|false].
   */
  def copyStream(
      in: InputStream,
      out: OutputStream,
