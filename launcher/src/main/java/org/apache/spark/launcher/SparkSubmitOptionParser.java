...
class SparkSubmitOptionParser {
  ...
  /**
   * 这是spark-submit选项的规范列表。数组中的每个条目都包含*同一选项的不同别名；
   * <p>
  ...
  /**
   * spark-submit识别的开关列表（不带参数的命令行选项）
   */
   ...
   * 有关可用选项的更正式描述，请参见SparkSubmitArguments.scala
   *
  ...
  protected final void parse(List<String> args) {
    ...
    for (idx = 0; idx < args.size(); idx++) {
      ...
      // opts spark支持的参数列表和指定的参数信息比对。如-- class
